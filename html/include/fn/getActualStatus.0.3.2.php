 ...
 * @date 2025.12.22
 * @filesource /include/fn/getActualStatus.0.3.1.php
 * @version 0.3.1
 * @bookmark getActualStatus()
...
function getActualStatus(bool $forceRebuild = false): array
{
	...
	if ($forceRebuild) {
		...
		// @bookmark Сознание каркаса конфигурации
		...
		$linkNames = isset($svxconfig['GLOBAL']['LINKS']) ? explode(",", $svxconfig['GLOBAL']['LINKS']) : [];
		$_links = [];
		foreach ($linkNames as $linkName) {
			...
			$logicEntries = explode(",", $svxconfig[$linkName]['CONNECT_LOGICS']);
			$_link_item = [
				'is_active' => false,
				'is_connected' => false,
				'start' => 0,
		...
		foreach ($logics as $_logic) {
			...
			if (
				...
				if (isset($svxconfig[$rxDeviceName]['TRANSMITTERS'])) {
					$transmitters = explode(",", $svxconfig[$rxDeviceName]['TRANSMITTERS']);
			...
			$item = [
				...
				'rx' => $rxProcessed,
				'tx' => $txProcessed,
				'macros' => $svxconfig[$_logic]['MACROS'] ?? '',
			...
			if (isset($svxconfig[$_logic]['MODULES'])) {
				$item['module'] = [];
				$moduleNames = explode(",", str_replace('Module', '', $svxconfig[$_logic]['MODULES']));
			...
			if ($hasDefaultTg || $hasMonitorTgs) {
				$item['talkgroups'] = [
					...
					'monitoring' => isset($svxconfig[$_logic]['MONITOR_TGS']) ?
						explode(",", $svxconfig[$_logic]['MONITOR_TGS']) : [],
					'temp_monitoring' => '0',
				];
			...
			if (preg_match('/Connected nodes:\s*(.+)$/', $logConnectedNode, $matches)) {
				...
				foreach ($callsigns as $fullCallsign) {
					if (!empty($fullCallsign)) {
						...
						if (preg_match('/^([A-Za-z]+[0-9]*[A-Za-z]*)/', $fullCallsign, $callMatch)) {
							$baseCallsign = $callMatch[1];
							$logic['connected_nodes'][$fullCallsign] = [
								'callsign' => $baseCallsign,
								'timestamp' => $nodesTimestamp, // Сохраняем время получения данных
							];
