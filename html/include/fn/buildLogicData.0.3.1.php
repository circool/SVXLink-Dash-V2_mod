 ...
 *               - 'active_module_nodes': узлы активного модуля
 ...
 * @since 0.3.1 Добавлено оригинальное (не усеченное) имя логики для правильного наименования блоков left_panel
 *              Добавлена проверка на пустые строки после формирования shortname
 * 
 * @filesource /include/fn/buildLogicData.0.3.1.php
 * 
 * 
	...
	foreach ($lp_logics as $logicName => $logic) {
		...
		if ($activeModule && !empty($activeModule['connected_nodes'])) {
			foreach ($activeModule['connected_nodes'] as $nodeName => $nodeData) {
				$nodeInfo = [
					'parent' => $activeModule['name'], // тут подумать как обновлять таблицу с данными в левой панели
					'name' => $nodeName,
					...
					'tooltip_start' => !empty($nodeData['start']) ?
						'<a class="tooltip" href="#"><span><b>' . getTranslation('Uptime') . ':</b>' . formatDuration(time() - $nodeData['start']) .
						(!empty($nodeData['type']) ? '<br>' . htmlspecialchars($nodeData['type']) : '') .
						(!empty($nodeData['callsign']) ? ' ' . htmlspecialchars($nodeData['callsign']) : '') . '</span>' : '',
