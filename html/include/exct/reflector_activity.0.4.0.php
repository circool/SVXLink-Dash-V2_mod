 ...
 * @since 0.4.0
 * Изменены id тумблеров для управления линками для версии 0.4.0 ws-сервера
 */
if (defined("DEBUG") && DEBUG) {
	$funct_start = microtime(true);
	$ver = 'reflector_activity.0.4.0';
...
<script>
	...
	function setLinkState(toggleElement) {
		...
		fetch(window.location.href, {
			...
			// @since 0.4 - не анализируем ответ во избежании сообщений о непонятном 
			.catch(() => {
				// Пока игнорируем все ошибки сети/сервера 
				// @todo понять кто отправляет в AJAX скрипт инициализации WS
			})
			// .then(response => {
			// 	if (!response.ok) {
