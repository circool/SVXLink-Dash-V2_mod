...
/**
 * @filesource /include/exct/keypad.0.2.1.php
...
$ver = 'keypad.php 0.2.1';
// if (defined("DEBUG") && DEBUG) dlog("$ver: init includes successfully", 4, "DEBUG");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_keypad'])) {
    ...
    error_log("AJAX: Session ID = " . session_id());
    error_log("AJAX: Elemens count = " . count($_SESSION));
    error_log("AJAX: Sessioon content: " . print_r($_SESSION, true));
    
    $cmd = $_SESSION['DTMF_CTRL_PTY'] ?? '';
    $keypad_activeLogics = getActiveLogic()['DTMF_CTRL_PTY'];
    // if ($keypad_activeLogics !== null) {
    //     foreach ($keypad_activeLogics as $_logic) {
    //         if (($_logic['is_active'] === false &&  $_logic['is_connected'] === false)
    //             || $_logic['type'] === 'Reflector'
    //         ) {
    //             // if (defined("DEBUG") && DEBUG) dlog("$ver: Пропускаю логику {$_logic['name']} как неподходящую", 4, "WARNING");
    //             continue;
    //         }
    //         if (isset($_logic['dtmf_cmd']) && !empty($_logic['dtmf_cmd'])) {
    //             $cmd = $_logic['dtmf_cmd'];
    //             break;
    //         }
    //     }
    ...
    if(!isset($_SESSION['DTMF_CTRL_PTY']) || is_null($_SESSION['DTMF_CTRL_PTY'])){
        ...
        // if (defined("DEBUG") && DEBUG) dlog('$ver: Команда управления DTMF не задана', 2, "WARNING");
...
$cmd = $_SESSION['DTMF_CTRL_PTY'];
// $kp_logics = getActiveLogic();
// if ($kp_logics !== false) {
//     foreach ($kp_logics as $_logic) {
//         if (($_logic['is_active'] === false && $_logic['is_connected'] === false) || $_logic['type'] === 'Reflector') {
//             dlog("Пропускаю логику {$_logic['name']} как неактивную ", 4, "DEBUG");
//             continue;
//         }

//         if (isset($_logic['dtmf_cmd']) && !empty($_logic['dtmf_cmd'])) {
//             dlog("Нашел команду управления ", 4, "DEBUG");
//             $cmd = $_logic['dtmf_cmd'];
//             break;
//         }
//     }
