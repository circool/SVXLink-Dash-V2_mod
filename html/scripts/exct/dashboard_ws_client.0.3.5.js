/**
 * @filesource /scripts/exct/dashboard_ws_client.0.3.5.js
 * @version 0.3.5
 * @date 2026.01.07
 ...
 * 
 * @since 0.3.5
 * - Добавлена обработка подключённых узлов модулей (EchoLink/Frn)
 * - Поддержка отображения последнего подключённого узла
 * - Автоматическое скрытие/показ блоков подключённых узлов
 * - Совместимость с сервером dashboard_ws_server.0.3.5.js
 */
...
class StatefulWebSocketClient {
	...
	init() {
		...
		this.debug('info', `Stateful WebSocket Client v0.3.5 initialized`);
		this.debug('info', `Compatible with server 0.3.5 (module node tracking support)`);
	}
	...
	processServerMessage(data) {
		switch (data.type) {
			...
			case 'module_node':
				this.debug('debug', 'module_node received from server');
				this.handleModuleNodeUpdate(data);
				break;
			...
			data.updates.forEach(update => {
				if (update.type === 'radio_status') {
					...
					if (update.logic) {
						...
						// Если модуль деактивирован, очищаем блок узлов
						if (update.state === 'deactivated') {
							this.clearModuleNodeDisplay(update.logic, update.module);
						}
					}
				}
			});
				...
				if (update.type === 'radio_status') {
					...
					if (update.logic) {
						if (update.state === 'activated' || update.state === 'connected') {
							...
							// Очистка блока подключенных узлов при деактивации модуля
							this.clearModuleNodeDisplay(update.logic, update.module);
						}
					}
				}
	...
	// ==================== ОБРАБОТКА УЗЛОВ МОДУЛЕЙ ====================

	/**
	 * Обработка сообщения о подключении/отключении узла модуля
	 */
	handleModuleNodeUpdate(data) {
		

		const { logic, module, action, node, uptime } = data;

		// Ищем элементы ТОЛЬКО по ID
		const table = document.getElementById(`${logic}modulesTable`);
		const header = document.getElementById(`${logic}moduleNodesHeader`);
		const content = document.getElementById(`${logic}moduleNodesContent`);

		if (!table || !header || !content) {
			this.debug('error', `Module nodes elements not found for ${logic}`);
			return;
		}

		if (action === 'connected') {
			// Показываем блок и заполняем
			table.classList.remove('hidden');
			header.textContent = module;

			const cleanName = node.replace(/\*/g, '');
			let nodeType = 'Node';

			if (node.startsWith('*') && node.endsWith('*')) nodeType = 'Conference';
			else if (node.endsWith('-L')) nodeType = 'Link';
			else if (node.endsWith('-R')) nodeType = 'Repeater';
			else if (module === 'Frn') nodeType = 'FrnServer';

			content.innerHTML = `
				<a class="tooltip" href="#">
					<span><b>Uptime:</b> ${uptime || '00:00:00'}<br>
					${nodeType} ${this.escapeHtml(cleanName)}</span>
					${this.escapeHtml(node)}
				</a>
			`;
			content.title = node;

		} else if (action === 'disconnected' || action === 'deactivated') {
			// Очищаем и скрываем при отключении или деактивации
			content.innerHTML = '';
			content.title = '';
			header.textContent = '';
		...
		this.debug('info', `${logic}: ${module} node ${action}: ${node}`);
	}

	/**
	 * Очистка блока отображения узлов модуля
	 */
	clearModuleNodeDisplay(logic, module) {
		const table = document.getElementById(`${logic}modulesTable`);
		const header = document.getElementById(`${logic}moduleNodesHeader`);
		const content = document.getElementById(`${logic}moduleNodesContent`);
		...
		if (table && header && content) {
			...
			header.textContent = '';
			table.classList.add('hidden');
			this.debug('info', `Cleared module node display for ${logic}:${module}`);
		}
	}
...
document.addEventListener('DOMContentLoaded', () => {
	console.log('DOM Content Loaded - Stateful WebSocket Client 0.3.5');
	...
	console.log('Creating Stateful WebSocket Client 0.3.5');
	...
	console.log('Stateful WebSocket Client 0.3.5 initialized');
});