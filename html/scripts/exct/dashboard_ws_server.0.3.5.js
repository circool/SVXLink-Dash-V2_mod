/**
 * @filesource /scripts/exct/dashboard_ws_server.0.3.5.js
 * @version 0.3.5
 * @date 2026.01.07
 ...
 * - Восстановление состояния при "потерянных" событиями
 * - Правильная логика: один активный модуль на логику
 * @since 0.3.5
 * - Изменение общей концепции отображения подключенных узлов EchoLink - отражаем только последний
 * - Из парсера удалены сырые данные
 * - Добавлена обработка имени узла для Frn
 * - Отправка событий module_node для очистки блоков при деактивации модулей
 */
		...
		this.patterns = [
			...
			// Frn: login stage 2 completed
			{
				...
				handler: (match) => {
					...
					return {
						type: 'module_connection',
						event: 'login_completed',       // ← ОСТАВЛЯЕМ как было!
						module_type: 'Frn',
						xml_data: match[2],             // ← ОСТАВЛЯЕМ для обратной совместимости
						server_name: serverName,        // ← ДОБАВЛЯЕМ распарсенное имя
						timestamp: match[1],
						raw: match[0]
					};
				}
...
class DeviceStateManager {
	...
	activateModule(logic, module, timestamp) {
		...
		// Если был другой активный модуль → деактивируем его и отправляем событие очистки
		if (existingState && existingState.module !== module) {
			...
			// Возвращаем событие очистки для старого модуля
			return {
				...
				module_node_data: {
					logic: logic,
					module: existingState.module,
					action: 'deactivated',
					node: existingState.node || existingState.server_name || '',
					moduleType: existingState.moduleType
				}
			};
		}
	...
	deactivateModule(logic, module) {
		...
		if (existingState && existingState.module === module) {
			...
			return {
				...
				// Добавляем данные для очистки блока узлов
				module_node_data: {
					logic: logic,
					module: module,
					action: 'deactivated',
	...
	processModuleConnectionEvent(event) {
		const { module_type, event: state, node, server_name, xml_data, timestamp } = event;
		...
		if (state === 'state_changed' || state === 'login_completed') {
			const connected = (state === 'state_changed' && event.state === 'connected') ||
				state === 'login_completed';

			if (connected) {
				return this.connectModule(logic, module, timestamp, node, server_name);
			} else {
	...
	connectModule(logic, module, timestamp, node, server_name) {
		const existingState = this.activeModules.get(logic);
		const now = Date.now();
		...
		if (!existingState) {
			...
			existingState.server_name = server_name;
...
class StatefulWebSocketServer {
	...
	sendWelcome(ws, clientId) {
		ws.send(JSON.stringify({
			...
			clientId: clientId,
			serverTime: Date.now(),
			message: 'Connected to Stateful WebSocket Server 0.3.5 with module tracking and uptime support'
		}));
	}
	...
	// ==================== ОТПРАВКА СОБЫТИЙ УЗЛОВ МОДУЛЕЙ ====================

	sendModuleNodeEvent(moduleNodeData) {
		if (!moduleNodeData) return 0;

		const { logic, module, action, node, moduleType } = moduleNodeData;

		// Получаем uptime
		let uptime = '00:00:00';
		const moduleState = this.stateManager.activeModules.get(logic);
		if (moduleState && moduleState.connectedAt && action === 'connected') {
			const uptimeMs = Date.now() - moduleState.connectedAt;
			uptime = this.stateManager.formatDuration(uptimeMs);
		}

		// Создаем сообщение module_node
		const moduleNodeMessage = {
			type: 'module_node',
			logic: logic,
			module: module,
			action: action,
		...
		const sentCount = this.broadcast(moduleNodeMessage);

		if (sentCount > 0) {
			log(`Sent module_node: ${logic}:${module} ${action} ${node || 'no node'} to ${sentCount} clients`, 'DEBUG');
		}
		...
		linesToProcess.forEach(line => {
			...
			if (event) {
				...
				// Обработка активации/деактивации модулей
					...
					if (result) {
						// Добавляем обновления для модуля
						if (result.updates) {
						...
						// Отправляем событие module_node если есть данные (особенно для deactivated)
						if (result.module_node_data) {
							this.sendModuleNodeEvent(result.module_node_data);
						}
					}
				}
				// Обработка подключения модулей
					...
					if (result) {
						...
						// Отправляем событие module_node если есть данные
						if (result.module_node_data) {
							this.sendModuleNodeEvent(result.module_node_data);
						}

						const state = event.state || (event.event === 'state_changed' ? event.state : event.event);
						log(`Module connection ${state}: ${event.module_type} ${result.module_node_data?.node || ''}`, 'INFO');
					}
				}
			}
		});
