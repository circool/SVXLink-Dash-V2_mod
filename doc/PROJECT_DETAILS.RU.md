...
3.1.1. Настройки в `settings.php`
  ...
  - константы для конфигурации svxlink
	...
	3.1.2.1. Выполняется чтение файла конфигурации svxlink и построение структуры компонентов (init.php), данные о конфигурации передаются в функцию getActualStatus.
...
Система использует гибридный подход: периодические AJAX-обновления с возможностью real-time через WebSocket. При отключенном WebSocket все обновления происходят через AJAX.
