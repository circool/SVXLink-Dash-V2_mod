...
Настройки определяющие цветовую схему 
	...
	- Дополнительная (приоритетная) - `/css/custom.css` 
...
#### 3.1.1. Технические настройки
    ...
    + настроки для realtime сервера
      - `WS_PORT`
      - `WS_PATH`
    + `USE_CACHE`         Глобально, использовать кеш при разборе лога (настоятельно рекомендуется)
...
### ['status'] : array
  ...
  - `callsign`: string (глобальный позывной)
	- `modules_history` : array (история активности модулей)
...
#### Структура modules_history
	
	array: [
	- `start` : int (время активации)
	- `end` : int (время деактивации)
	- `module` : string (имя модуля)
	- `logic` : string (имя логики)
	]
...
│   ├── ui_settings.php                 # Настройки интерфейса
...
│   ├── custom.css                        # Дополнения к основному набору
