...
Исторические сводки вычисляются соответствующими функциями отдельно для каждого блока. Данные рассчитываются путем анализа журнала svxlink в пределах последней активной сессии. Интервал обновления таких данных устанавливается константой `SLOW_UPDATE_INTERVAL` -> `UPDATE_INTERVAL` .
