...
История - данные о активности - последние несколько событий для соответствующих режимов, напр. последняя локальная активность, последняя сетевая активность (вызовы пришедшие из интернета, от рефлекторов напр. ) 

Исторические сводки вычисляются соотв. функциями отдельно для каждого блока. 
Данные рассчитываются путем агализа журнала svxlink, 
...
# Сервис состояния реального времени

## Общее назначение

Предоставляет двустороннюю коммуникацию между сервером SvxLink и веб-интерфейсом Dashboard в реальном времени. Сервер мониторит журнал SvxLink, парсит события и отправляет обновления состояния всем подключенным клиентам через WebSocket. Клиент отображает статусы устройств и автоматически поддерживает соединение.

## Архитектура

### Сервер (Node.js WebSocket сервер, порт 8080)

**Основные компоненты:**

1. **WebSocket сервер** (`StatefulWebSocketServer`)
   - Управляет подключениями клиентов
   - Рассылает обновления состояния всем активным клиентам
   - Автоматически завершает работу при отсутствии клиентов (таймаут 15 секунд)

2. **Менеджер состояния устройств** (`DeviceStateManager`)
   - Отслеживает активные устройства (transmitter/squelch)
   - Управляет активностью talkgroup рефлекторов
   - Вычисляет длительность активности
   - Формирует обновления для клиентов

3. **Парсер лога** (`SimpleParser`)
   - Анализирует строки журнала SvxLink
   - Распознаёт основные события:
     - Transmitter ON/OFF
     - Squelch OPEN/CLOSED  
     - Talkgroup activity (talker_start/talker_stop)

4. **Мониторинг журнала**
   - Использует `tail -F` для отслеживания лога в реальном времени
   - Буферизация строк для групповой обработки
   - Автоматический перезапуск при ошибках

**Особенности версии 0.3.3:**
- Поддержка событий рефлекторов (talkgroup_activity)
- Автоматическое обновление длительности активности каждую секунду
- Формирование обновлений для позывных и статусов TX при работе с рефлекторами
- Интеллектуальное управление мониторингом (запуск/остановка при наличии клиентов)

**Особенности версии 0.3.4:**
- Поддержка событий модулей (активация, подключение)
- @todo 0.3.5 Добавить сброс/актуализацию подключенных узлов для модулей EchoLink/Frn


### Клиент (JavaScript WebSocket клиент)

**Основные компоненты:**

1. **StatefulWebSocketClient**
   - Подключается к серверу WebSocket
   - Обрабатывает входящие сообщения от сервера
   - Применяет обновления к DOM элементам
   - Автоматические переподключения при разрыве

2. **Интерфейс управления**
   - Единая кнопка статуса в навигационной панели
   - Визуальная индикация состояния подключения
   - Контекстные действия (отключение/перезагрузка)

3. **Система восстановления**
   - Автоматические попытки переподключения
   - При полном отключении - перезагрузка страницы для запуска сервера через `init.php`
   - Дебаунс для предотвращения множественных перезагрузок

**Состояния подключения:**
- `connected` - подключен, кнопка отключает
- `disconnected` - отключен, кнопка перезагружает страницу
- `connecting` - в процессе подключения
- `error` - ошибка подключения, кнопка перезагружает
- `timeout` - таймаут подключения, кнопка перезагружает

## Принцип работы

### Инициализация
1. Сервер запускается на порту 8080 (обычно через `init.php`)
2. Клиент при загрузке страницы устанавливает WebSocket соединение
3. Сервер отправляет текущее состояние всех активных устройств

### Мониторинг событий
1. Сервер постоянно мониторит лог SvxLink
2. При обнаружении события (transmitter, squelch, talkgroup) парсер извлекает данные
3. Менеджер состояния обновляет внутренние структуры
4. Формируются обновления DOM для клиента

### Обновление интерфейса
1. Клиент получает сообщение с обновлениями
2. Находит соответствующие DOM элементы по ID
3. Обновляет контент, классы и data-атрибуты
4. Для активных устройств показывает длительность работы

### Управление соединением
- Кнопка статуса предоставляет единую точку управления
- В зависимости от состояния выполняет разные действия
- При отключении клиента сервер останавливает мониторинг через 15 секунд
- При перезагрузке страницы PHP скрипт перезапускает сервер
...
### Серверная конфигурация (дефолтная):
```javascript
{
  host: '0.0.0.0',
  port: 8080,
...
- `{device}_StatusTX` - статус передатчика
- `{device}_StatusRX` - статус приёмника  
- `{device}Callsign` - позывной (для рефлекторов)

### CSS классы:
- `active-mode-cell` - активное состояние (приём/передача)
- `inactive-mode-cell` - неактивное состояние
- `menuwebsocket-connected` - кнопка при подключении
- `menuwebsocket-disconnected` - кнопка при отключении

## Особенности реализации

### Масштабируемость:
- Сервер поддерживает множественных клиентов
...
### Расширяемость:
- Модульная структура парсера для добавления новых типов событий
- Гибкая система обновлений DOM
- Конфигурируемые параметры соединения

Система обеспечивает минимальную нагрузку на сервер SvxLink, так как использует только чтение лог-файла, и предоставляет актуальное состояние системы всем подключённым веб-клиентам в реальном времени.
